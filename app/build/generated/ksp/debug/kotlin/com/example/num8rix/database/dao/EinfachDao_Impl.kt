...
public class EinfachDao_Impl(
  __db: RoomDatabase,
) : EinfachDao {
  private val __db: RoomDatabase

  private val __insertAdapterOfEinfach: EntityInsertAdapter<Einfach>
  init {
    this.__db = __db
    this.__insertAdapterOfEinfach = object : EntityInsertAdapter<Einfach>() {
      protected override fun createQuery(): String =
          "INSERT OR ABORT INTO `einfach` (`id`,`unsolvedString`,`layoutString`,`solutionString`,`alreadySolved`) VALUES (nullif(?, 0),?,?,?,?)"

      protected override fun bind(statement: SQLiteStatement, entity: Einfach) {
  ...
  public override suspend fun insert(einfach: Einfach): Unit = performSuspending(__db, false, true)
      { _connection ->
    __insertAdapterOfEinfach.insert(_connection, einfach)
  }

  public override suspend fun getSolutionStringById(itemId: Int): String? {
    val _sql: String = "SELECT solutionString FROM einfach WHERE id = ?"
  ...
  public override suspend fun getBySolvedStatus(solvedStatus: Boolean): List<Einfach> {
    val _sql: String = "SELECT * FROM einfach WHERE alreadySolved = ?"
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
        ...
        val _result: MutableList<Einfach> = mutableListOf()
        while (_stmt.step()) {
          val _item: Einfach
          val _tmpId: Int
          ...
          _item =
              Einfach(_tmpId,_tmpUnsolvedString,_tmpLayoutString,_tmpSolutionString,_tmpAlreadySolved)
          _result.add(_item)
        }
        _result
      } finally {
  ...
  public override suspend fun getRandomUnsolved(): Einfach? {
    val _sql: String = "SELECT * FROM einfach WHERE alreadySolved = 0 ORDER BY RANDOM() LIMIT 1"
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        val _cursorIndexOfId: Int = getColumnIndexOrThrow(_stmt, "id")
  ...
  public override suspend fun puzzleExists(unsolvedString: String): Boolean {
    val _sql: String = "SELECT COUNT(*) > 0 FROM einfach WHERE unsolvedString = ?"
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
  ...
  public override suspend fun getTotalCount(): Int {
    val _sql: String = "SELECT COUNT(*) FROM einfach"
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        val _result: Int
  ...
  public override suspend fun getSolvedCount(): Int {
    val _sql: String = "SELECT COUNT(*) FROM einfach WHERE alreadySolved = 1"
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        val _result: Int
  ...
  public override suspend fun getById(itemId: Int): Einfach? {
    val _sql: String = "SELECT * FROM einfach WHERE id = ? LIMIT 1"
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
  ...
  public override suspend fun getAllPuzzles(): List<Einfach> {
    val _sql: String = "SELECT * FROM einfach"
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        val _cursorIndexOfId: Int = getColumnIndexOrThrow(_stmt, "id")
  ...
  public override suspend fun checkPuzzleExists(unsolvedString: String, layoutString: String):
      Boolean {
    val _sql: String =
        "SELECT COUNT(*) > 0 FROM einfach WHERE unsolvedString = ? AND layoutString = ?"
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
  ...
  public override suspend fun updateSolvedStatus(itemId: Int, newStatus: Boolean) {
    val _sql: String = "UPDATE einfach SET alreadySolved = ? WHERE id = ?"
