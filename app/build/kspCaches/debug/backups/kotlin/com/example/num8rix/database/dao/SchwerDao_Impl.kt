...
public class SchwerDao_Impl(
  __db: RoomDatabase,
) : SchwerDao {
  private val __db: RoomDatabase

  private val __insertAdapterOfSchwer: EntityInsertAdapter<Schwer>
  init {
    this.__db = __db
    this.__insertAdapterOfSchwer = object : EntityInsertAdapter<Schwer>() {
      protected override fun createQuery(): String =
          "INSERT OR ABORT INTO `schwer` (`id`,`unsolvedString`,`layoutString`,`solutionString`,`alreadySolved`) VALUES (nullif(?, 0),?,?,?,?)"

      protected override fun bind(statement: SQLiteStatement, entity: Schwer) {
  ...
  public override suspend fun insert(schwer: Schwer): Unit = performSuspending(__db, false, true) {
      _connection ->
    __insertAdapterOfSchwer.insert(_connection, schwer)
  }

  public override suspend fun getSolutionStringById(itemId: Int): String? {
    val _sql: String = "SELECT solutionString FROM schwer WHERE id = ?"
  ...
  public override suspend fun getBySolvedStatus(solvedStatus: Boolean): List<Schwer> {
    val _sql: String = "SELECT * FROM schwer WHERE alreadySolved = ?"
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
        ...
        val _result: MutableList<Schwer> = mutableListOf()
        while (_stmt.step()) {
          val _item: Schwer
          val _tmpId: Int
          ...
          _item =
              Schwer(_tmpId,_tmpUnsolvedString,_tmpLayoutString,_tmpSolutionString,_tmpAlreadySolved)
          _result.add(_item)
        }
        _result
      } finally {
  ...
  public override suspend fun getRandomUnsolved(): Schwer? {
    val _sql: String = "SELECT * FROM schwer WHERE alreadySolved = 0 ORDER BY RANDOM() LIMIT 1"
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        val _cursorIndexOfId: Int = getColumnIndexOrThrow(_stmt, "id")
  ...
  public override suspend fun puzzleExists(unsolvedString: String): Boolean {
    val _sql: String = "SELECT COUNT(*) > 0 FROM schwer WHERE unsolvedString = ?"
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
  ...
  public override suspend fun getTotalCount(): Int {
    val _sql: String = "SELECT COUNT(*) FROM schwer"
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        val _result: Int
  ...
  public override suspend fun getSolvedCount(): Int {
    val _sql: String = "SELECT COUNT(*) FROM schwer WHERE alreadySolved = 1"
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        val _result: Int
  ...
  public override suspend fun getById(itemId: Int): Schwer? {
    val _sql: String = "SELECT * FROM schwer WHERE id = ? LIMIT 1"
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
  ...
  public override suspend fun getAllPuzzles(): List<Schwer> {
    val _sql: String = "SELECT * FROM schwer"
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        val _cursorIndexOfId: Int = getColumnIndexOrThrow(_stmt, "id")
  ...
  public override suspend fun checkPuzzleExists(unsolvedString: String, layoutString: String):
      Boolean {
    val _sql: String =
        "SELECT COUNT(*) > 0 FROM schwer WHERE unsolvedString = ? AND layoutString = ?"
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
  ...
  public override suspend fun updateSolvedStatus(itemId: Int, newStatus: Boolean) {
    val _sql: String = "UPDATE schwer SET alreadySolved = ? WHERE id = ?"
